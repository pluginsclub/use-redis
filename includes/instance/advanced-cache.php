 ...
 *
 * redis.php creates a symbolic link to this file from the wp-content directory and enables page caching in wp-config.php
 * 
 */
...
class Redis_Page_Cache {
	...
	public function __construct() {
		self::$redis_host = get_option('object_cacher_redis_server', '127.0.0.1');
		self::$redis_port = get_option('object_cacher_redis_port', '6379');
	}
	...
	public static function cache_init() {
		...
		header( 'X-ROC-Cache-Status: miss' );
		...
		if ( ! empty( $_SERVER['HTTP_AUTHORIZATION'] ) ) {
			$request_hash['unique']['oc-auth-header'] = $_SERVER['HTTP_AUTHORIZATION'];
		}
		...
		if ( self::$debug ) {
			header( 'X-ROC-Cache-Key: ' . self::$request_hash );
		}
		...
		list( $cache, $lock ) = $redis->mGet( array(
			sprintf( 'roc-%s', self::$request_hash ),
			sprintf( 'roc-%s-lock', self::$request_hash ),
		) );
		...
		if ( is_array( $cache ) && ! empty( $cache ) ) {
			...
			if ( self::$debug ) {
				header( 'X-ROC-Cache-Time: ' . $cache['updated'] );
				header( 'X-ROC-Cache-Flags: ' . implode( ' ', $cache['flags'] ) );
			}

			$redis->multi();
			$redis->zRangeByScore( 'roc-expired-flags', $cache['updated'], '+inf', array( 'withscores' => true ) );
			$redis->zRangeByScore( 'roc-deleted-flags', $cache['updated'], '+inf', array( 'withscores' => true ) );
			...
			if ( $expired && $serve_cache ) {
				...
				if ( ! $lock ) {
					$lock = $redis->set( sprintf( 'roc-%s-lock', self::$request_hash ), true, array( 'nx', 'ex' => 30 ) );
					if ( $lock ) {
			...
			if ( $serve_cache ) {
				...
				header( 'X-ROC-Cache-Status: ' . $status );

				if ( self::$debug )
					header( sprintf( 'X-ROC-Cache-Expires: %d', self::$ttl - ( time() - $cache['updated'] ) ) );
	...
	private static function can_fcgi_regenerate() {
		return ( php_sapi_name() == 'fpm-fcgi' && function_exists( 'fastcgi_finish_request' ) && function_exists( 'oc_sapi_headers_clean' ) );
	}
...
public static function get_redis() {
    ...
    // Retrieve Redis host and port from wp-config.php
	$config_file = ABSPATH . 'wp-config.php';
	if (file_exists($config_file)) {
		$config_contents = file_get_contents($config_file);
		$matches = array();
		preg_match("/define\(\s*['\"]OBJECT_CACHER_REDIS_SERVER['\"]\s*,\s*['\"]([^'\"]+)['\"]\s*\)/", $config_contents, $matches);
		if (!empty($matches[1])) {
			$redis_host = $matches[1];
		}
		preg_match("/define\(\s*['\"]OBJECT_CACHER_REDIS_PORT['\"]\s*,\s*['\"]([^'\"]+)['\"]\s*\)/", $config_contents, $matches);
		if (!empty($matches[1])) {
			$redis_port = $matches[1];
		}
	}

	$redis = new Redis;
	$connect = $redis->connect($redis_host, $redis_port);
	...
	/**
	 * Parse config from oc-user-config.php or $redis_page_cache_config global.
	 */
	private static function maybe_user_config() {
		global $redis_page_cache_config;
		$oc_user_config = function_exists( 'oc_user_config' ) ? oc_user_config() : array();
		...
		foreach ( $keys as $key ) {
			if ( isset( $oc_user_config['page_cache'][ $key ] ) ) {
				self::$$key = $oc_user_config['page_cache'][ $key ];
	...
	public static function output_buffer( $output ) {
		...
		foreach ( headers_list() as $header ) {
			...
			// Never store X-ROC-Cache-* headers in cache.
			if ( strpos( strtolower( $key ), 'x-roc-cache' ) !== false )
				continue;
		...
		if ( $cache || self::$fcgi_regenerate ) {
			...
			if ( $cache ) {
				// Okay to cache.
				$redis->set( sprintf( 'roc-%s', self::$request_hash ), $data );
			} else {
				// Not okay, so delete any stale entry.
				$redis->delete( sprintf( 'roc-%s', self::$request_hash ) );
			}

			$redis->delete( sprintf( 'roc-%s-lock', self::$request_hash ) );
			$redis->exec();
		}
	...
	public static function maybe_clear_caches() {
		...
		if ( ! empty( self::$flags_expire ) )
			$sets['roc-expired-flags'] = self::$flags_expire;

		if ( ! empty( self::$flags_delete ) )
			$sets['roc-deleted-flags'] = self::$flags_delete;
	...
	public static function _add_action_compat() {
		...
		$GLOBALS['wp_filter']['clean_post_cache'][10]['oc-page-cache'] = array(
			'function' => array( __CLASS__, 'clean_post_cache' ), 'accepted_args' => 1 );
		$GLOBALS['wp_filter']['transition_post_status'][10]['oc-page-cache'] = array(
			'function' => array( __CLASS__, 'transition_post_status' ), 'accepted_args' => 3 );
		$GLOBALS['wp_filter']['template_redirect'][100]['oc-page-cache'] = array(
